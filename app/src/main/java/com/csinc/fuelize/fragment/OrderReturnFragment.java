    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        try {
            ...
            for (int i = 0; i < orderDataList.size(); i++) {
                if (orderDataList.get(i).getStatus().equalsIgnoreCase(Constants.REACHED_AT_RO) || orderDataList.get(i).getStatus().equalsIgnoreCase(Constants.REACHED_AT_RO_MANUALLY) || orderDataList.get(i).getStatus().equalsIgnoreCase(Constants.ORDER_RETURN)) {
                    orderPendingDataList.add(orderDataList.get(i));
                }
            }
            if (orderPendingDataList.size() > 0) {
                mAdapter = new OrdersRecyclerAdapter(orderPendingDataList, mContext, 2);
