    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        try {
            ...
            for (int i = 0; i < orderDataList.size(); i++) {
                if (orderDataList.get(i).getStatus().equalsIgnoreCase(Constants.ORDER_START) || orderDataList.get(i).getStatus().equalsIgnoreCase(Constants.ORDER_DOWNLOADED) || orderDataList.get(i).getStatus().equalsIgnoreCase(Constants.ORDER_CREATED) || orderDataList.get(i).getStatus().equalsIgnoreCase(Constants.REACHED_AT_CUSTOMER) || orderDataList.get(i).getStatus().equalsIgnoreCase(Constants.REACHED_AT_CUSTOMER_MANUALLY)) {
                    orderPendingDataList.add(orderDataList.get(i));
                }
            }
            if (orderPendingDataList.size() > 0) {
                mAdapter = new OrdersRecyclerAdapter(orderPendingDataList, mContext, 0);
